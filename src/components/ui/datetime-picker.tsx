...
export type DateTimePickerProps = {
  ...
  /**
   * date-fns locale applied to all internal `format()` calls (month names, AM/PM, display).
   * @default undefined (date-fns default = enUS)
   */
  locale?: Locale;
  /**
   * Placeholder shown in the trigger when there is no value selected.
   * @default 'Pick a date'
   */
  placeholder?: string;
  /**
   * Label for the "done" button in the popover footer.
   * @default 'Done'
   */
  doneLabel?: string;
  /**
   * Label for the timezone indicator in the popover footer.
   * @default 'Timezone:'
   */
...
export function DateTimePicker({
  ...
  const dislayFormat = useMemo(() => {
    if (!displayValue) return placeholder ?? 'Selecciona fecha y hora';
    return format(
      displayValue,
      `${!hideTime ? 'MMM' : 'MMMM'} d, yyyy${!hideTime ? (use12HourFormat ? ' hh:mm:ss a' : ' HH:mm:ss') : ''}`,
      locale ? { locale } : undefined
    );
  }, [displayValue, hideTime, use12HourFormat, locale, placeholder]);
          ...
          <div className="text-md font-bold ms-2 flex items-center cursor-pointer">
            <div>
              <span onClick={() => setMonthYearPicker(monthYearPicker === 'month' ? false : 'month')}>
                {format(month, 'MMMM', locale ? { locale } : undefined)}
              </span>
              <span className="ms-1" onClick={() => setMonthYearPicker(monthYearPicker === 'year' ? false : 'year')}>
                {format(month, 'yyyy', locale ? { locale } : undefined)}
              </span>
            </div>
          ...
          <div className="flex flex-row-reverse items-center justify-between">
            ...
            {timezone && (
              <div className="text-sm">
                <span>{timezoneLabel ?? 'Zona horaria:'}</span>
...
function MonthYearPicker({
  ...
  const months = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      months.push({ value: i, label: format(new Date(0, i), 'MMM', locale ? { locale } : undefined), disabled });
    }
    return months;
  }, [value, locale]);
...
function TimePicker({
  ...
  const display = useMemo(() => {
    ...
    return format(value, arr.join(':') + (use12HourFormat ? ' a' : ''), locale ? { locale } : undefined);
